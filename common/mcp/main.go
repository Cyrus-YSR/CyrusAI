...
import (
	...
	mcpclient "gopherai-mcp/client"
	mcpserver "gopherai-mcp/server"
)
...
func main() {
	...
	if *mode == "" {
		fmt.Println("Error: 您必须指定模式，使用 -mode (server 或 client)")
		flag.Usage()
		os.Exit(1)
	}
	...
	if *mode == "server" {
		...
		// 运行客户端
		if *city == "" {
			fmt.Println("Error: 您必须指定城市名称，使用 -city")
			flag.Usage()
			os.Exit(1)
		}
